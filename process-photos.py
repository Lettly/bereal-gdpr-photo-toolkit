...
print(
    STYLING["BOLD"]
    + "\nDo you want to access advanced settings or run with default settings?"
    + STYLING["RESET"]
)
print(
...
if advanced_settings == "yes":
    ...
    while convert_to_jpeg not in ["yes", "no"]:
        convert_to_jpeg = (
            input(
                STYLING["BOLD"]
                + "\n1. Do you want to convert images from WebP to JPEG? (yes/no): "
                + STYLING["RESET"]
            )
            .strip()
            .lower()
        ...
        if convert_to_jpeg == "no":
            print(
                "Your images will not be converted. No additional metadata will be added."
            )
    ...
    print(
        STYLING["BOLD"]
        + "\n2. There are two options for how output files can be named"
        + STYLING["RESET"]
        + "\n"
    ...
    while keep_original_filename not in ["yes", "no"]:
        keep_original_filename = (
            input(
                STYLING["BOLD"]
                + "Do you want to keep the original filename in the renamed file? (yes/no): "
                + STYLING["RESET"]
            )
            .strip()
            .lower()
...
if convert_to_jpeg == "no" and create_combined_images == "no":
    print(
        "You chose not to convert images nor do you want to output combined images.\n"
...
def update_exif(image_path, datetime_original, location=None, caption=None):
    try:
        ...
        exif_dict["Exif"][piexif.ExifIFD.DateTimeOriginal] = datetime_original.strftime(
            "%Y:%m:%d %H:%M:%S"
        )
        ...
        if location and "latitude" in location and "longitude" in location:
            ...
            gps_ifd = {
                ...
                piexif.GPSIFD.GPSLatitude: _convert_to_degrees(
                    abs(location["latitude"])
                ),
                piexif.GPSIFD.GPSLongitudeRef: (
                    "E" if location["longitude"] >= 0 else "W"
                ),
                piexif.GPSIFD.GPSLongitude: _convert_to_degrees(
...
def update_iptc(image_path, caption):
    try:
        ...
        )  # Use force=True to create IPTC data if it doesn't exist
...
def combine_images_with_resizing(primary_path, secondary_path):
    ...
    resized_secondary_image = secondary_image.resize(
    ...
    outline_layer = Image.new(
        "RGBA", combined_image.size, (0, 0, 0, 0)
    )  # Transparent layer for drawing the outline
    ...
    draw.rounded_rectangle(
        outline_box, corner_radius + outline_size, fill=(0, 0, 0, 255)
    )
...
if create_combined_images == "yes":
    ...
    for primary_path, secondary_path in zip(primary_images, secondary_images):
        ...
        update_exif(
            combined_image_path, primary_taken_at, primary_location, primary_caption
        )
        ...
        if convert_to_jpeg == "yes":
            ...
            update_exif(
                converted_path, primary_taken_at, primary_location, primary_caption
            )
            ...
            if converted_path is None:
                logging.error(
                    f"Failed to convert combined image to JPEG: {combined_image_path}"
                )
        print("")
...
print(
    STYLING["BOLD"]
    + "Removing backup files left behind by iptcinfo3"
    + STYLING["RESET"]
)
